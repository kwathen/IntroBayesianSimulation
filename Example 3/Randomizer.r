...
#   Function to get the treatment for a patient.
#   Description: 
#       This randomizer utilizes the probbility that the standard treatment (S)
#       is "better" than the experimental (E) to adapt the randomizaiton proability
...
#
#   Input:
#       dMinRandProb: the mimimum randomization probability that is desired.
...
GetTreatmentAdaptiveRandomization <- function( dMinRandProb, dExponent, nQtyPats, nMinQtyOfPats,  dProbSGrtE ) 
{
    ...
    else
    {
        ...
        #We also need to implemnt the use of dExponent and dMinRandProb 
        dRandProbS <- (dProbSGrtE^dExponent) / ( dProbSGrtE^dExponent + (1 - dProbSGrtE )^dExponent )
        
        # The following 2 lines make sure that the randomization proabilities are in the interval [dMinRandProb, 1-dMinRandProb]
        # This could be acomplished in 1 line combining but is less readable and more difficult to follow.  
